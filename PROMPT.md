...
Проект `InjusticeMod`: оверлей-меню для игры Injustice 2 Mobile 6.7.1. Хукает одну
нативную функцию через `MSHookFunction` и рисует плавающее окно UIKit.
...
2. **Не менять логику** в `hook_SetCurrentHealth`, `isPlayerCharacter` и
   `IMPassthroughWindow.hitTest:`. Последнее особенно: без него оверлей
