...
## 8. Что из этого стало авто-фармом

Из всего перечисленного выше автоматизируется ровно одно: **последовательность
действий игрока**. Бои идут по-настоящему, отчёт уходит на сервер штатно,
награды начисляет сервер. Ничего не подделывается — убирается ожидание между
боями.

Реализованная цепочка (адреса и их вывод — в [../HANDOFF.md](../HANDOFF.md),
раздел «Авто-фарм соло-рейдов»):

| Шаг | Точка | Откуда |
|---|---|---|
| сводка по боссу открыта | `0x1FFEC44` | виртуальный метод `USoloRaidManagementMenu` |
| запуск боя | `0x1E4C0DC` | `URaidBossSummaryWindow::OnStartButtonClicked` |
| предбой → бой | `0x1F48668` → `0x1F49A94` | уже было: `UPreFightMenu` общий для режимов |
| завершение боя | `0x1B4CFB4` → `0x1B41710` | уже было |
| экран наград | `0x1F3957C` → `0x1F39914` | уже было |
| почта | `0x1E9B244` → `mClaimAllButton +0x3C0` | `UInboxMenu` |

Ключевое, что позволило не писать половину заново: `UPreFightMenu` —
**общий** для кампании, арены и рейдов (в нём `SetRaidBossData`,
`SetPipRefreshTimer`, `UPreFightMenuCachedState_SoloRaid`), поэтому середина
цепочки уже была готова.

**Ограничители взяты из этого же файла.** Пороги античита конфигурируются
`TMap<EGameModeType, FCheatDetectionData>` — отдельно на режим, а
`lastBattlesDealtDamage` и `LastWonBattleValidationDataPerLevel` означают, что
сервер смотрит на ряд боёв, а не на один. Поэтому пауза перед авто-завершением
боя в рейдах своя (по умолчанию 12 с против 3 с в кампании), и это настройка,
а не константа.

Останов цикла — по данным самой игры, а не по таймеру: сумма
`CommonPipsCount`/`BonusPipsCount`/`PremiumPipsCount` равна нулю, либо
`URaidManagementMenu::CanFight` (виртуальный слот `0x658`) вернул `false`.
